...
public final class SnowFlakeIdGenerator {
     ...
     * 初始时间截 (2019-09-01)
     */
    private static final long INITIAL_TIME_STAMP = 1567267200000L;
     ...
     * 模块id
     */
    private static Integer MODULE_ID;
    ...
    private SnowFlakeIdGenerator() {
        if (MODULE_ID == null) {
            ...
            this.datacenterId = MODULE_ID % (MAX_DATACENTER_ID + 1);
        }
            ...
            if (network == null) {
                ...
                if (null != mac) {
                    ...
                    id = ((lowMask & (long) mac[mac.length - 1])
                            | (highMask & (((long) mac[mac.length - Constants.TWO]) << Constants.EIGHT)))
                            >> Constants.SIX;
...
//            throw new RuntimeException("当前时间小于上一次记录的时间戳！");
        ...
        if (lastTimestamp == timestamp) {
            ...
            sequence = ThreadLocalRandom.current().nextLong(Constants.ZERO, Constants.NINE);
...
//        return System.currentTimeMillis();
    ...
    public long[] parseId(long id) {
        long[] arr = new long[Constants.FIVE];
        arr[Constants.FOUR] = ((id & diode(UNUSED_BITS, TIME_STAMP_BITS)) >> TIMESTAMP_OFFSET);
        arr[Constants.ZERO] = arr[Constants.FOUR] + INITIAL_TIME_STAMP;
        arr[Constants.ONE] = (id & diode(UNUSED_BITS + TIME_STAMP_BITS, DATACENTER_ID_BITS)) >> DATACENTERID_OFFSET;
        arr[Constants.TWO] = (id & diode(UNUSED_BITS + TIME_STAMP_BITS + DATACENTER_ID_BITS, WORKER_ID_BITS)) >> WORKERID_OFFSET;
        arr[Constants.THREE] = (id & diode(UNUSED_BITS + TIME_STAMP_BITS + DATACENTER_ID_BITS + WORKER_ID_BITS, SEQUENCE_BITS));
        return arr;
    }

    public String formatId(long id) {
        long[] arr = parseId(id);
        String tmf = DateUtil.formatDate(arr[0], "yyyy-MM-dd HH:mm:ss.SSS");
        return String.format("%s, @(%d,%d), #%d", tmf, arr[Constants.ONE], arr[Constants.TWO], arr[Constants.THREE]);
    }
    ...
    public static void setModuleId(Integer id) {
        MODULE_ID = id;
    }

    public static void setModuleId(String idStr) {
        if (StringUtils.isNotEmpty(idStr)) {
            try {
                MODULE_ID = Integer.valueOf(idStr);
            } catch (NumberFormatException e) {
                log.error("moduleId error:", e);
            }
        }
    }
